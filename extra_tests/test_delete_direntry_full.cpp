...
int main(int argc, char *argv[]) {
    ...
    status = fs_create("user1", "/dir/3", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/4", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/5", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/6", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/7", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/8", 'f');
    assert(!status);

    status = fs_delete("user1", "/dir/8");
    assert(!status);

    status = fs_create("user1", "/dir/8", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/9", 'f');
    assert(!status);

    status = fs_delete("user1", "/dir/9");
    assert(!status);
